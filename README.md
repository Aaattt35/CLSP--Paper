...
**Note:** When executed in GitHub Actions, the CSV output is created in the runner environment and is not persisted in the repository by default. In this workflow, the file is stored under the `run-model` directory and uploaded as a CSV artifact, allowing it to be retrieved and downloaded after the workflow completes.
